...
public class SemiautomaticPackingRepair implements IRealization{

	private static final Logger log = Logger.getLogger(SemiautomaticPackingRepair.class.getName());
	...
	public TDataList startProcess(String filePath, TDataList input, TDataList outputHolder) {
		...
		try {
			...
			log.severe("PRS.001.02: FileNotFoundException has Occurred.");
		} catch (NullPointerException e) {
			log.severe("PRS.001.01: NullPointerException has Occurred.");
		} catch (Exception e) {
			log.severe("PRS.001.00: Unknown Exception has Occurred - " + e);
		}
		return outputHolder;
	}
}