...
public class ActivitiExecutor implements IRealization{
	...
	public ActivitiExecutor() {
		this.bpmnFilePath = null;
		this.processName = null;
	}
	
	public ActivitiExecutor(String bpmnFilePath, String processName) {
	...
	public TDataList startProcess(TDataList input, TDataList outputHolder) {
		...
		try {
			repositoryService.createDeployment().addInputStream("cmp_process.bpmn20.xml",
					new FileInputStream(this.bpmnFilePath)).deploy();
			RuntimeService runtimeService = processEngine.getRuntimeService();
			Map<String, Object> variableMap = new HashMap<String, Object>();
			//Initial Process Inputs
			variableMap.put("orderID", "OD153728DE");

			ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(this.processName, variableMap);
			
			//Check whether the manual activities need to be run
			boolean processtype = this.decideProcessType(this.processName);
			if(processtype){
